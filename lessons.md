...
## Step summary lost to context compaction between agent return and next turn (RET-36)
Step 5's Commit agent ran correctly — 8 well-formed, atomic, correctly-attributed commits, exactly matching `commit.md`'s own rules. But the coordinator-log entry documenting that spawn was never written: the session's context got summarized between the agent's return and the next turn, and whatever would have written the Step 5 entry never happened. A later continuation of the same session found 8 unexplained commits with no log trail and, reasonably but wrongly, concluded a serious protocol violation had occurred — commits made without the Commit agent. That alarm was only corrected by directly auditing the actual git state (author/committer on every commit, granularity, message quality against `commit.md`'s own checklist) rather than trusting the log's absence as evidence either way. The near miss: an absent on-disk summary is not proof a step didn't happen, only proof it wasn't recorded — and the fix almost went the other way, treating the silence as confirmed violation and reaching for corrective action before verifying. The rule this sharpens: write the step's on-disk summary as the very first thing after receiving an agent's result, before anything else, specifically because a compaction boundary can fall anywhere afterward and there is no way to know in advance where.

## Multi-repo commit passes need a fresh per-repo branch check (RET-36)
The same session spanned three repos (AI service, frontend, backend) in one pipeline run. Mid-session, work meant for the frontend's feature branch ended up committed directly to its `main` instead — discovered only when the user pointed at it directly, not caught by anything in the flow itself. Investigating further, the frontend's feature branch didn't even exist by that point (recreated later from the commit that already had the work on `main`). `commit.md`'s "never commit to main/master" rule is fine on its own — the gap is that nothing in a multi-repo commit pass forces re-checking each repo's current branch independently, right before that repo's commits are made, not just once at session start. A single `git branch --show-current` per repo, run again immediately before staging anything in it, would have caught this before it happened rather than after.
