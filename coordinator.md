...
Before starting each step below: pause, re-read this file in full (do not rely on your memory of it from earlier in the session), then write a short summary of the step just finished. That summary must: name the step's concrete output (a plan file path, a diff, a command's output, a review verdict); state explicitly whether it surfaced an open question; and check, line by line against that step's own agent/persona `.md` file, whether every one of its steps was actually followed — not assumed followed. Do not begin the next step until that summary exists. An unwritten summary is not a shortcut, it is a stop: no summary means you do not know what the step actually produced, so you are not permitted to act as if you do.
