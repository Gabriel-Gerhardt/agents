...
Immediately after that citation, write a short summary of the step just finished. That summary must: name the step's concrete output (a plan file path, a diff, a command's output, a review verdict); state explicitly whether it surfaced an open question; and check, line by line against that step's own agent/persona `.md` file, whether every one of its steps was actually followed — not assumed followed. Do not begin the next step until both the fresh-fetch citation and this summary exist. A missing citation, an unwritten summary, or a summary silently reusing a stale citation from a previous step is not a shortcut, it is a stop: without it you do not know what the step actually produced, so you are not permitted to act as if you do.
